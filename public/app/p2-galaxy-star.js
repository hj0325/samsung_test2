(function () {
  ...
  function drawActiveMerged(ctx, frameCount, activeFrame) {
    ctx.clearRect(0, 0, CANVAS_PX, CANVAS_PX);

    var cycleT = (activeFrame % ACTIVE_CYCLE) / ACTIVE_CYCLE;
    var blend = cycleT < 0.5
      ? easeInOutQuint(cycleT * 2)
      : easeInOutQuint(2 - cycleT * 2);

    var cluster = getClusterState();
    var merged = getMergedState();
    var f = interpolateFrameState(cluster, merged, blend);
  ...
  GalaxyStarController.prototype.mount = function () {
    ...
    var existing = this.starEl.querySelector('.p2-galaxy-star__canvas');
    if (existing) {
      this.canvas = existing;
    } else {
      this.canvas = document.createElement('canvas');
      this.canvas.className = 'p2-galaxy-star__canvas';
      this.canvas.width = CANVAS_PX;
      this.canvas.height = CANVAS_PX;
      this.canvas.setAttribute('aria-hidden', 'true');
      this.starEl.textContent = '';
      this.starEl.appendChild(this.canvas);
    }
  ...
  GalaxyStarController.prototype._draw = function () {
    ...
    if (this.phase === 'active') {
      drawActiveMerged(this.ctx, this.frameCount, this.activeFrame);
      return;
    }
    if (this.phase === 'merge') {
      drawStarScene(this.ctx, this.frameCount, this.mergeFrame, this.reactiveBlast);
      return;
    }
    drawStarScene(this.ctx, this.frameCount, this.idleFrame, this.reactiveBlast);
  };
  ...
  window.P2GalaxyStar = {
    ...
    syncFromCanvas: function (canvasEl) {
      ...
      if (active) current.setPhase('active');
      else current.setPhase('idle');
    }
  };
})();
